...
class Files extends Controller
{
    ...
    public function file()
    {
        ...
        if(isset($_GET['fileaction']) && $_GET['fileaction'] == 'delete' && $_SESSION['rf_user'] === 'admin') {
            $this->_files->deleteFile($fileID);
            \Helpers\Url::redirect('files');
        }

        if(ctype_digit($_GET['commentid']) && $_GET['commentaction'] == 'delete' && $_SESSION['rf_user'] === 'admin') {
            $this->_files->deleteComment($_GET['commentid']);
            \Helpers\Url::redirect('files/' . $fileID);
        }
    ...
    public function sendAppeal()
    {
        ...
        $mail->body(
            '<p>Жалоба на файл: ' . 'http://rocketfiles.zz.mu/files/' . \Helpers\Data::html($_POST['fileid']) . '</p>' .
